...
namespace PCA9531_I2CPWM
{
    public partial class frmI2CPWM : Form
    {
        ...
        private void frmI2CS_Load(object sender, EventArgs e)
        {
            ...
            cmbLed2.SelectedIndex = 0;
            cmbLed3.SelectedIndex = 0;
            cmbLed4.SelectedIndex = 0;
            cmbLed5.SelectedIndex = 0;
            cmbLed6.SelectedIndex = 0;
            cmbLed7.SelectedIndex = 0;
        ...
        {
            byte value = (byte)(cmbLed0.SelectedIndex + cmbLed1.SelectedIndex * 4 + cmbLed2.SelectedIndex* 16 + cmbLed3.SelectedIndex * 64);
            byte addr = GetAddress(false);
            // write to LED Selector register
            CommObj.Send(new byte[] { addr, 0x5, value }, 0);
        }

        private void cmbLed4_SelectedIndexChanged(object sender, EventArgs e)
        {
            byte value = (byte)(cmbLed4.SelectedIndex + cmbLed5.SelectedIndex * 4 + cmbLed6.SelectedIndex * 16 + cmbLed7.SelectedIndex * 64);
            byte addr = GetAddress(false);
            // write to LED Selector register
            CommObj.Send(new byte[] { addr, 0x6, value }, 0);
        }
    }
