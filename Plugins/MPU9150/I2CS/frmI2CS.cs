    ...
    public partial class frmI2CS : Form
    {
        ...
        private string ReadSensor()
        {
            try
            {
                ...
                byte[] value = CommObj.Send(new byte[] { addr,107, 0}, 0);
                value = CommObj.Send(new byte[] { addr, 59}, 6);
                if (value != null )
                {
                    if (value.Length == 6)
                    {
                ...
                value = CommObj.Send(new byte[] { addr, 67 }, 6);
                if (value != null)
                {
                    if (value.Length == 6)
                    {
                        lbGX.Text = (value[0] * 256 + value[1]).ToString();
                        lbGY.Text = (value[2] * 256 + value[3]).ToString();
                        lbGZ.Text = (value[4] * 256 + value[5]).ToString();
                    }
                }
